...
author: lucio
date: 2024-06-17
description: Functions to specify the symmetry in the observation and action space for GO2.
'''
...
def _transform_policy_obs_left_right(env: ManagerBasedRLEnv, obs: torch.Tensor) -> torch.Tensor:
    ...
    for the go2 robot. Additionally, if height-scan data is present, it is flipped
    ...
    # joint pos
    # 关节位置
    obs[:, 9:21] = _switch_go2_joints_left_right(obs[:, 9:21])

    # joint vel
    # 关节速度
    obs[:, 21:33] = _switch_go2_joints_left_right(obs[:, 21:33])

    # last actions
    # 上一步动作
    obs[:, 33:45] = _switch_go2_joints_left_right(obs[:, 33:45])
...
def _transform_policy_obs_front_back(env: ManagerBasedRLEnv, obs: torch.Tensor) -> torch.Tensor:
    ...
    and flipping the joint positions, joint velocities, and last actions for the go2 robot.
    ...
    # joint pos
    # 关节位置
    obs[:, 9:21] = _switch_go2_joints_front_back(obs[:, 9:21])

    # joint vel
    # 关节速度
    obs[:, 21:33] = _switch_go2_joints_front_back(obs[:, 21:33])

    # last actions
    # 上一步动作
    obs[:, 33:45] = _switch_go2_joints_front_back(obs[:, 33:45])
    ...
    # # joint pos
    # # 关节位置
    # obs[:, 12:24] = _switch_go2_joints_front_back(obs[:, 12:24])
    # # joint vel
    # # 关节速度
    # obs[:, 24:36] = _switch_go2_joints_front_back(obs[:, 24:36])
    # # last actions
    # # 上一步动作
    # obs[:, 36:48] = _switch_go2_joints_front_back(obs[:, 36:48])
